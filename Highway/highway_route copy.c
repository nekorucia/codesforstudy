...
short int found[CITIES+1];
int n = CITIES+1; 
// int index[CITIES+1]; //지나간 노드를 추적하는 배열을 만들고 싶은데 현재 구현불가 
int start,goal; 

void shortestPath(int v, int dist[], int n, short int)
{
    int i,u,w;
    for(i=0;i<n;i++)
    {
    ...
    for(i=0;i<n-2;i++)
    {
        ...
        for(w=0;w<n;w++)
            if(!found[w])
                if(dist[u]+highway[u][w]<dist[w])
                    dist[w]=dist[u]+highway[u][w];
    }
}
...
int main(void)
{
    ...
    printf("확인, 스타트와 골 : %d %d \n",start,goal); // start와 goal이 제대로 설정되었는지 확인, 제대로 들어감. 

    INPUT(start,goal); // 출발지와 도착지의 key 값을 넣음

    printf("확인, 배열 초기화 : %f %f \n",highway[32][32],highway[35][32]); // double형 highway 전역 배열이 제대로 초기화됐는지 확인, 제대로 돌아감. 
